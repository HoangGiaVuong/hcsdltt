...
def get_actual_cost_from_db(conn, query_params, plan_id):
    ...
    # GIẢ ĐỊNH: Bạn đang muốn tối ưu hóa cho truy vấn này
    sql_template = """
    SELECT COUNT(*)
    FROM Fact_Trips
    ...
    plan_settings = {
        ...
        'plan_index_vendor': [
            ...
            # Giả sử Postgres đủ thông minh để chọn index 'vendor_id_fk'
            # Nếu không, bạn có thể cần các gợi ý (hints) phức tạp hơn
            # Nhưng việc tắt 'seqscan' thường là đủ.
        ],
        'plan_index_tip': [
            "SET enable_indexscan = on;",
            "SET enable_bitmapscan = on;",
            "SET enable_seqscan = off;"
            # Tương tự, giả sử có index trên 'tip_amount' và Postgres sẽ dùng nó
            # khi 'seqscan' bị tắt.
        ]
    }

    # Regex để trích xuất thời gian thực thi từ EXPLAIN ANALYZE
    # Ví dụ: "Execution Time: 123.456 ms"
    ...
    with conn.cursor() as cur:
        try:
            ...
            # Chúng ta dùng psycopg2 để truyền tham số an toàn
            ...
            if cost_ms is None:
                ...
                print("Output của EXPLAIN:", explain_output)
                return None
...
def generate_historical_logs(n_samples=5000):
    ...
    # GIẢ ĐỊNH: Lấy các giá trị min/max thực tế từ CSDL
    # (Bạn nên điều chỉnh các truy vấn này nếu cần)
    with conn.cursor() as cur:
...
# (Phần này giống hệt như code trước, vì nó chỉ nhận DataFrame)

def train_cost_model(log_df):
    """
    Huấn luyện mô hình RandomForest để dự đoán 'actual_cost'
    từ các đặc trưng của truy vấn và kế hoạch.
    """
    print("Đang huấn luyện mô hình dự đoán chi phí...")
...
# (Phần này cũng giống hệt code trước)
...
class MLQueryOptimizer:
    ...
    def __init__(self, cost_model_pipeline):
        ...
        # Giả sử chúng ta chỉ tối ưu hóa giữa 3 kế hoạch này
        # (Chúng ta cần đảm bảo có index trên vendor_id_fk và tip_amount)
    ...
    def get_optimal_plan(self, query_params):
        ...
        query_params: dict, ví dụ {'vendor_id': 5, 'tip_amount': 10}
        """
        ...
        print("--- Chi phí dự đoán bởi ML Model (đơn vị: ms) ---")
...
if __name__ == "__main__":
    ...
    # Cảnh báo: Việc này sẽ chạy HÀNG NGÀN truy vấn EXPLAIN ANALYZE
    # trên CSDL của bạn. Chỉ chạy trên môi trường development/staging.
    # Giảm n_samples nếu cần (ví dụ: 1000)
    ...
    if not historical_logs_df.empty:
        log_filename = "pg_query_logs.csv"
        historical_logs_df.to_csv(log_filename, index=False)
        print(f"Đã lưu log lịch sử vào '{log_filename}'")

        # BƯỚC 2: HUẤN LUYỆN MÔ HÌNH
        # (Nếu bạn đã có file log, bạn có thể comment BƯỚC 1
        # và load file log tại đây)
        # historical_logs_df = pd.read_csv("pg_query_logs.csv")
        ...
        if ml_cost_model:
            ...
            # Trường hợp 1: Truy vấn có vẻ chọn lọc (tip thấp)
            ...
            # Trường hợp 2: Truy vấn có vẻ không chọn lọc (tip cao)
